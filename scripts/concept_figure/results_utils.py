...
def plot_c2st_success_frontier(results_root, methods, threshold=0.75, agg="mean", savepath=None):
    """
    ...
    for ii, method in enumerate(methods):
        df = load_table(results_root, "c2st", method, agg=agg)
        frontier = {}
        for dim in df.columns:
...
def plot_runtime_success_frontier(results_root, methods, threshold=0.75, agg="mean", savepath=None):
    """
    ...
    for ii, method in enumerate(methods):
        df_c2st = load_table(results_root, "c2st", method, agg=agg)
        ...
        for dim in df_c2st.columns:
            ...
            if len(ok_budgets) > 0:
                ...
                runtimes = []
                budget_path = os.path.join(results_root, f"D_{dim}", f"{method}_{chosen_budget}")
...
if __name__ == "__main__":
    results_root = "./../../results/mog_benchmark/two_modes_distractors/"
    savepath = "./../../results/mog_benchmark/two_modes_distractors"
