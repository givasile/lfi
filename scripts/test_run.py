...
obs = lfi.observations.Zeros(dim_y = simulator_dim_y, nof_observations=obsevation_nof_obs)
observation = obs.sample(nof_obs=obsevation_nof_obs, dim_y=simulator_dim_y) + 3

# define inference
inference = lfi.inference.from_sbi.NPEASingleRound(
    prior=prior.return_sbi_object(),
    simulator=simulator.sample_pytorch,
