...
# Plot ground truth
utils.plot_gt_2d(savefig="paper/figures/concept_image_distractors_gt.pdf")
...
y_0 = np.concatenate([np.ones((dim,)) * 5, np.zeros((dim_distractors,))])
...
utils.plot_posterior_kde_2D(samples_romc, savefig="paper/figures/concept_image_distractors_romc_kde.pdf")
...
simulator = simulators.CasesGaussianDistractors(
    ...
    sigma_1=1.,
    sigma_2=1.,
    dim_distractors=distractors_dim
)
y_0 = np.concatenate([np.ones((dim,)) * 5, np.zeros((distractors_dim,))])
prior = priors.Uniform(low=-15., high=15., dim=dim)
...
samples_rromc = samples
utils.plot_posterior_kde_2D(samples_rromc, savefig="paper/figures/concept_image_distractors_rromc_kde.pdf")
...
sim = simulators.CasesGaussianDistractors(dim=dim, sigma_1=1.0, sigma_2=1.0, dim_distractors=distractors_dim)
...
utils.plot_posterior_kde_2D(samples_nle, savefig="paper/figures/concept_image_distractors_nle_kde.pdf", bw_method=.2)
...
samples_npe = posterior.sample((5000,), x=torch.Tensor(y_0))
time_npe = time.time() - tic
print(f"NPE took {time_npe} seconds.")

utils.plot_posterior_kde_2D(samples_npe, savefig="paper/figures/concept_image_distractors_npe_kde.pdf")
...
utils.plot_posterior_kde_1d(
    ...
    samples_npe,
    samples_nle,
    savefig="paper/figures/concept_image_distractors.pdf"
)
