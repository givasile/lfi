...
def posterior_gt(N, y_0, D, nof_samples, sigma=1.):
    ...
    center_1 = y_0[:D] if y_0.ndim == 1 else y_0[0, :]
    center_2 = - y_0[D:] if y_0.ndim == 1 else - y_0[1, :]
    rv1 = multivariate_normal(mean=center_1, cov=covariance_matrix)
    rv2 = multivariate_normal(mean=center_2, cov=covariance_matrix)
    return np.concatenate([rv1.rvs(N // 2), rv2.rvs(N // 2)])
...
def plot_posterior_gt(N_y, savefig):
    ...
    covariance_matrix = np.diag([1] * 2) / np.sqrt(N_y)
    center_1 = np.ones(2) * 5
    center_2 = np.ones(2) * -5
    rv1 = multivariate_normal(mean=center_1, cov=covariance_matrix)
    rv2 = multivariate_normal(mean=center_2, cov=covariance_matrix)

    plt.contourf(X, Y,(rv1.pdf(pos) + rv2.pdf(pos)) / 2, cmap='viridis')
...
def save_metrics(metrics):
    ...
    df.to_csv("results/exp_multi_observation_MoG.csv", index=False)


def load_metrics():
    # if file exists, load it, otherwise do nothing
    if os.path.exists("results/exp_multi_observation_MoG.csv"):
        df = pd.read_csv("results/exp_multi_observation_MoG.csv")
...
for i, N_y in enumerate(conf_D_list): #   ([2, 5, 10, 15, 20]):
    ...
    sim_flat = simulators.CasesGaussianMultiSamples(dim=dim, sigma_1=1.0, sigma_2=1.0, nof_samples=N_y)
    ...
    plot_posterior_samples_2D(gt_samples,
                              "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_gt_samples.pdf")
    plot_posterior_gt(N_y, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_gt_posterior.pdf")
    ...
    if conf_run_r2omc:
        ...
        plot_posterior_samples_2D(samples, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_r2omc_samples.pdf")
        plot_posterior_kde_2D(samples, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_r2omc_kde.pdf")
    ...
    if conf_run_romc:
        ...
        plot_posterior_samples_2D(samples_romc, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_romc_samples.pdf")
        plot_posterior_kde_2D(samples_romc, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_romc_kde.pdf")
    ...
    if conf_run_nle:
        ...
        plot_posterior_samples_2D(samples_nle, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_nle_samples.pdf")
        plot_posterior_kde_2D(samples_nle, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_nle_kde.pdf")
    ...
    if conf_run_npe:
        ...
        for _ in range(nof_epochs):
            ...
            # x = simulator(theta)
            _ = inferer.append_simulations(torch.Tensor(theta), torch.Tensor(x), proposal=proposal).train(
                training_batch_size=tr_batch_size,
                max_num_epochs=500,
            )
        ...
        plot_posterior_samples_2D(samples_npe, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_npe_samples.pdf")
        plot_posterior_kde_2D(samples_npe, "paper/figures/exp_multi_observation_MoG_N_" + str(N_y) + "_npe_kde.pdf")
...
# plot
if conf_save_plot:
    plot_c2st(metrics, "paper/figures/exp_multi_observation_MoG_c2st.pdf")
    plot_runtime(metrics, "paper/figures/exp_multi_observation_MoG_runtime.pdf")
else:
