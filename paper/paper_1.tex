...
\begin{abstract}
Parameter inference for complex stochastic simulators remains challenging due to intractable likelihood functions.
%
While many simulation-based methods extend Bayesian inference to simulators, they often become particularly slow in challenging settings such as high-dimensional parameter spaces or with partially uninformative outputs, where they require large simulated datasets.
%
We introduce a novel method for differentiable simulators that achieves accurate posterior inference in significantly faster runtimes.
%
Our approach builds on the Optimization Monte Carlo framework by transforming the simulator into differentiable optimization problems.
%
This enables rapid navigation to high-density posterior regions without wasting `blind' simulations on low-probability areas.
%
The method's efficiency is further enhanced through a \texttt{JAX}-based implementation that parallelizes critical components.
%
Extensive experiments show that our method effectively handles challenging scenarios, including high-dimensional parameter spaces, uninformative output dimensions, multiple observations, multimodal posteriors, and real-world applications.
%
In these cases, it achieves comparable accuracy to existing methods while requiring substantially reduced computational time.
\end{abstract}
...
A variety of methods, collectively known as Likelihood-Free (LFI) or Simulation-Based (SBI) Inference \citep{lintusaari2017fundamentals, sisson2018handbook, cranmer2020frontier}, have been developed to infer simulator parameters.
These include sampling-based approaches, like Approximate Bayesian Computation \citep[ABC,][]{Marin2012}, the use of summary statistics \citep{Chen2021a, Chen2023}, parametric modeling \citep{wood2010statistical, price2018bayesian}, and ratio estimation \citep{thomas2022likelihood, durkan2020contrastive}.
More recently, substancial progress has come from neural-based methods, which use deep neural networks to learn summary statistics \citep{radev2020bayesflow}, or employ neural surrogate models to estimate the posterior \citep{greenberg2019automatic, papamakarios2016fast} or the likelihood function \citep{papamakarios2019sequential}.

% but unfortunatelly neural-based methods are data-hungry and yet expensive to train
Neural network–based approaches have greatly expanded the applicability of SBI to complex problem settings.
However, they remain highly data-intensive; they demand massive simulated datasets to train the neural estimators and achieve satisfactory performance.
As both data simulation and neural network training are expensive, neural-based methods incur significant runtimes, thereby limiting experimental flexibility and practical deployment.

% Two particular challenges: high-dimensional parameters and distractors
This challenge is particularly pronounced in two scenarios.
First, in high-dimensional parameter spaces; SBI methods suffer from the curse of dimensionality, as they rely on populating both parameter and data spaces with samples \citep{lintusaari2017fundamentals, sisson2018handbook, cranmer2020frontier}.
Second, in distractor settings; In complex simulators, parameters and outputs often interact in intricate ways, so certain outputs may only depend weakly, or not at all, on certain parameters \citep{Saltelli2008, MonsalveBravo2022}.
Such uninformative outputs act as ``distractors'' complicating inference \citep{lueckmann2021benchmarking}.
As illustrated in Figure~\ref{fig:concept_example}, under these scenarios existing neural-based approaches struggle when operating under a limited simulation budget.
...
We here propose a new LFI method that uses gradient information to address these challenges.
Building on the Robust Optimization Monte Carlo framework \citep[ROMC,][]{ikonomov2020robust},
we reformulate the stochastic simulator as a collection of deterministic optimization problems and apply gradient-descent to obtain posterior samples.
As in standard optimization, gradients are crucial for efficiently navigating in high-dimensional parameter spaces.
In addition, they enable sensitivity analyses which allows us to identify and filter out distractor dimensions, thereby improving the quality of the inference.

% Our contributions
The ROMC framework provides the theory for casting inference as deterministic optimization, but, with two crucial limitations:
it lacks a strategy for defining an appropriate distance function in problems with distractors or multiple iid observations (Section~\ref{sec:romc-limitations}),
and
its latest implementation~\citep{gkolemis2023extendable} does not exploit gradients or other computational accelerations limiting its applicability to low-dimensional problems.
%
We address these challenges with \rromc, an SBI method within the ROMC family, which, when the simulator is differentiable,
...
\begin{itemize}
 ...
 % We here briefly introduce Robust Optimization Monte Carlo (ROMC), which forms the theoretical basis of our method, and discuss its limitations.

 \subsection{Neural-based methods are data-intensive}

 Neural–based SBI have demonstrated strong performance on complex simulators.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{./figures/sbibm/slcp/c2st}
        \caption{SLCP: C2ST}
        \label{subfig:sbi_slcp_c2st}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{./figures/sbibm/slcp/runtime}
        \caption{SLCP: Log runtime}
        \label{subfig:sbi_slcp_runtime}
    \end{subfigure}\\
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{./figures/sbibm/slcp_distractors/c2st}
        \caption{SLCP-dist: C2ST}
        \label{subfig:sbi_slcp_dist_c2st}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{./figures/sbibm/slcp_distractors/runtime}
        \caption{SLCP-dist: Log runtime}
        \label{subfig:sbi_slcp_dist_runtime}
    \end{subfigure}
    \caption{SLCP and SLCP with distractors benchmark results.}
    \label{fig:sbi_experiments_slcp_c2st}
\end{figure}

\begin{figure}
    \centering
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{figures/sbibm/slcp/pairwise_posterior_total.png}
        \caption{SLCP total}
        \label{subfig:sbi_slcp_total}
    \end{subfigure}
    % \begin{subfigure}[b]{0.32\linewidth}
    %     \includegraphics[width=\linewidth]{figures/sbibm/slcp/pairwise_posterior_accepted.png}
    %     \caption{SLCP accepted}
    %     \label{subfig:sbi_slcp_accepted}
    % \end{subfigure}
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{figures/sbibm/slcp/pairwise_posterior_selected.png}
        \caption{SLCP selected}
        \label{subfig:sbi_slcp_selected}
    \end{subfigure}
    \caption{SLCP pairwise posteriors}
    \label{fig:sbi_experiments_slcp}
\end{figure}

As its name suggests, the SLCP task is designed to have a Simple Likelihood but a Complex Posterior~\citep{papamakarios2019sequential, greenberg2019automatic}.

The prior is uniform over five parameters $\thb \in \mathcal{U}(-\mathbf{3}, \mathbf{3})^5$ and the observations consist of four two-dimensional points drawn from a Gaussian distribution whose mean and variance are nonlinear functions of~$\theta$.
This setup yields a posterior with four symmetric modes and sharp vertical cut-offs.
SLCP with distractors augments the original SLCP task by adding 92 additional non-informative outputs, resulting in a total of 100 output dimensions.

Figure~\ref{fig:sbi_experiments_slcp_c2st} illustrates the \CtwoST~score and runtime of \rromc~and all methods reported by \citet{lueckmann2021benchmarking} for SLCP (top row) and SLCP with distractors (bottom row).
\rromc~achieves an accurate posterior approximation (\CtwoST~score between $0.7$ and $0.8$) with a small simulation budget of $10^3$ samples and a runtime of a few seconds.
All other methods require a budget of at least $10^4$ (and often $10^5$) samples to reach a similar score, leading to runtimes of several minutes or even hours.
The results are more pronounced in the distractor version, where most methods fail to reach a \CtwoST~score below $0.8$ even with a budget of $10^5$ samples.

Figure~\ref{fig:sbi_experiments_slcp} illustrates the rationale behind the multiple observation weighting scheme in \rromc (Eq.\ref{eq:rromc_weight}).
Figure~\ref{subfig:sbi_slcp_total} shows the pairwise posteriors using all proposal samples generated by \rromc, while Figure~\ref{subfig:sbi_slcp_selected} using only the ones that are finally selected.
The proposal samples are the ones that are in $\epsilon$-distance to at least one observation, while the selected samples are the ones that are in $\epsilon$-distance to all observations and thus represent the posterior.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.49\linewidth}
        ...
        \caption{2-moons total}
        \label{subfig:sbi_two_moons_c2st}
    \end{subfigure}
    % \begin{subfigure}[b]{0.32\linewidth}
    %     \includegraphics[width=\linewidth]{figures/sbibm/two_moons/runtime}
    %     \caption{2-moons accepted}
    %     \label{subfig:sbi_two_moons_accepted}
    % \end{subfigure}
    \begin{subfigure}[b]{0.49\linewidth}
        \includegraphics[width=\linewidth]{figures/sbibm/two_moons/pairwise_posterior.png}
        \caption{2-moons selected}
        \label{subfig:sbi_two_moons_posterior}
    \end{subfigure}
    \caption{2-moons pairwise posteriors}
...
The 2-moons task tests whether the methods can accurately estimate a posterior with both global (bimodality) and local (crescent shape) structure.
...
The parameters $\thb=(\theta_1, \theta_2)$ are two-dimensional and the prior is $\thb \sim \mathcal{U}(\mathbf{-1}, \mathbf{1})$. This creates a posterior with two half-moon shape modes.

Figure~\ref{fig:sbi_two_moons} shows the \CtwoST~score (left) and the pairwise posterior (right) of \rromc~and all methods reported by \citet{lueckmann2021benchmarking}.
\rromc~has a \CtwoST~score close to $0.5$ even with a simulation budget of $10^3$ samples, while the rest of the methods need a budget of $10^5$ or more to achieve a similar score and perform much worse for smaller budgets.
The pairwise posterior in Figure~\ref{subfig:sbi_two_moons_posterior} shows that \rromc~accurately captures the two crescent-shaped modes of the posterior.

\subsection{Image-based inference}
\label{subsec:image-based}

We consider a high-dimensional image-based inference problem, where the goal is to infer the parameters of a random simulator that distorts and adds Gaussian noise to an input image.
The problem tests the ability of \rromc to handle very high-dimensional data ($D_y = 28 \times 28 = 784$), even beyond the dimensionality of conventional SBI problems.
The simulator is $\mathbf{y} = \text{transform}(\thb) + \boldsymbol{\epsilon}$, where $\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0}, 0.1^2 \mathbf{I})$ and $\text{transform}(\thb)$ applies a transformations to the base image.
We test two types of transformations: (a) pixel-wise intensity changes, where $\thb \in \mathbb{R}^{28 \times 28}$ and each parameter controls the intensity of one pixel; (b) a checkerboard filter, where an edge-detection filter is applied to the base image.

Figure~\ref{fig:pixelwise} summarizes the results.
In the top row, we show the initial clean image.
In the second row the pixel-wise distorted observation and in the third and fourth rows the posterior mean and standard deviation obtained by \rromc (case (a)).
In the last three rows, we show the observation, posterior mean and standard deviation for the checkerboard filter (case (b)).
In both cases, \rromc~obtains a posterior mean that is relatively close to the clean image, in terms of optical similarity, which means that the posterior mode is close to parameter $\thb^*$ that generated the observation.
