...
%
Extensive experiments, including high-dimensional parameter spaces, uninformative outputs, multiple observations and multimodal posteriors show that our method consistenly matches, and often exceeds, the accuracy of state-of-the-art approaches, while reducing the runtime by a substantial margin.
\end{abstract}
...
However, accuracy comes with greater complexity, such as a large numbers of free parameters or a high-dimensional output.
...
Since both data generation, using simulation calls, and neural network training are computationally costly, these methods often incur significant runtimes, which in turn restricts experimental flexibility and hinders practical deployment.
...
As shown in Figure~\ref{fig:concept_example}, accurate inference with existing neural-based approaches comes only at the cost of many simulations and substantial runtimes.
...
We overcome these challenges and present \rromc, an SBI method that when the simulator is differentiable,
...
\begin{itemize}
 ...
 \rromc~is systematically compared to state-of-the-art SBI methods, demonstrating accurate posterior inference at a fraction of the computational cost in a wide range of scenarios.

 \section{Background and Motivation}

 Neural-based SBI methods are data-intensive, with their accuracy depending on access to large simulated datasets.
 This leads to two main sources of computational cost:
 (a) generating datasets from complex simulators, and
 (b) training deep neural estimators on high-volume data.
 Prior work~\citep{lueckmann2021benchmarking} has noted that training is often the more time-consuming step.
 Further details are provided in Appendix~\ref{app-sec:compute-budget}.
Moreover, with modern frameworks, such as JAX, which enable extensive vectorization, simulation can be accelerated, further shifting the bottleneck toward training.
This motivates us to search for a method that sidesteps the bottleneck of separate neural estimator training.
