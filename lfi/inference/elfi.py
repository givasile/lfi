...
class SMCRejection(InferenceBase):
    ...
    def fit(self, budget: int=1000, fit_kwargs: dict=None, verbose: int = 1):
        ...
        default_kwargs.update((fit_kwargs or {}))
        self.nof_iterations = default_kwargs["nof_iterations"]
        # budget = total simulator calls, split equally across populations
        batch_size = budget // self.nof_iterations
        self.inference_method = elfi.SMC(self.d, batch_size=batch_size)

    def sample(self, nof_samples: int=100, sample_kwargs: dict=None, verbose: int = 1):
        default_kwargs = {
            "nof_iterations": self.nof_iterations,
        }
