...
if __name__ == "__main__":
    # general settings
    savefig = True
    dir_path = "../../results/images/image_checkerboard"
    ...
    for i in range(10):
        ...
        posterior_samples_mask = np.where(posterior_samples_mask < 0.3, 0, posterior_samples_mask)

        # outputs
        plt.figure(figsize=(5, 5))
        yy = posterior_samples_mask.mean(axis=0).reshape(28, 28)
        yy[yy < 0.2] = 0
        plt.imshow(yy, cmap='gray', vmin=0, vmax=1)
        plt.clim(0, 1)
        plt.axis('off')
        if savefig:
